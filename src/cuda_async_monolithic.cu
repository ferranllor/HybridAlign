...
AlignmentResult gpu_align_async_monolithic(Graph graph, Graph cudaGraph, Sequence sequence)
{
    ...
    for (int d = 0; d < num_levels; d++)
    {
        ...
        compute_dp_gpu_async_monolithic<<<gridDim, blockDim, 0, compute_stream>>>(act, sequence, sequence_rev);
        ...
        size_t level_matrix_elements = 0;
        for (int i = 0; i < nodes_per_level[d]; i++) {
            int n = node_offset + i;
            level_matrix_elements += (size_t)(sequence.size + 2) * (graph.nodes[n].sequence.size + 2);
        }

        if (level_matrix_elements > 0) {
            int first_node_in_level = node_offset;
            
            cudaMemcpyAsync(graph.nodes[first_node_in_level].dp_matrix, 
                            device_nodes_pointers[first_node_in_level].dp_matrix, 
                            level_matrix_elements * sizeof(DTYPEMATRIX), 
    ...
    return compute_traceback_gpu_async_monolithic(graph, sequence);
}
