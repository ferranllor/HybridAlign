...
AlignmentResult gpu_align_shared_mem(Graph graph, Graph cudaGraph, Sequence sequence)
{
    ...
    for (int d = 0; d < num_levels; d++)
    {
        ...
        int dynamic_shared_mem_bytes = sizeof(DTYPEMATRIX) * (graph.nodes[d].sequence.size + sequence.size + 2 + (BLOCKSIZE * 3));
...
__global__ void compute_dp_gpu_shared_mem(Node* node, Sequence sequence, Sequence sequence_rev)
{
    ...
    __shared__ DTYPEMATRIX dpBuffers[nBuffers][BLOCKSIZE];
    ...
    // I will keep two versions. One with the use of shared mem and one without, but bothe with the indexing, as I plan to do a comparison.
    ...
    for (int startM = 0; startM < M; startM += BLOCKSIZE) {
        ...
        
        {
            ...
            int total = min(begin + stripe_height, end) - (begin);

            if (threadIdx.x == 0)
                dpBuffers[bufferPrev][0] = dp[startPrev + begin + 0 - 1];

            for (int k = threadIdx.x; k <= total; k += blockDim.x) {
                dpBuffers[bufferPrevPrev][k] = dp[startPrevPrev + begin + k - 1];
                dpBuffers[bufferPrev][k + 1] = dp[startPrev + begin + k];
            }

            __syncthreads();
        };

        startCurr = get_diag_start_device(startM + 1, M, N);
        startPrev = get_diag_start_device(startM, M, N);
        startPrevPrev = 0;
        ...
        for (; d <= l_min; ++d) {
            ...
            int total = min(begin + stripe_height, end);

            if (threadIdx.x == 0) {
                dpBuffers[bufferAct][0] = prevCol[d];
                dpBuffers[bufferAct][total + 1] = topRow[d - 1];
            }

            for (int k = begin + threadIdx.x; k <= total; k += blockDim.x) {
                int j = (k_start + k) - 1;
                int i = M - d + (k_start + k);
        ...
        for (; d <= (M+N); ++d) {
            ...
            for (int k = begin + threadIdx.x; k <= total; k += blockDim.x) {
                int j = (k_start + k + off_curr) - 1;
                int i = M - d + (k_start + k + off_curr);
