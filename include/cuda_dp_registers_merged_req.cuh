...
// The DP core of the registers_merged_req version, on ONE node, for ONE warp. It lives in a header
// for the same reason the other cores do: the build has no device side linking, so the multi read
// version cannot reach a __device__ function sitting in another .cu.
//
// col_off is where this warp's last column lives. The single sequence version passes 0; the multi
// read version passes the offset of the read it is aligning.

#ifndef CUDA_DP_REGISTERS_MERGED_REQ_H
#define CUDA_DP_REGISTERS_MERGED_REQ_H
...
// How many diagonals share one halo load and one handover store.
#ifndef REQ_BLOCK
#define REQ_BLOCK WARP_SIZE
#endif

__device__ static void compute_dp_node_registers_merged_req(Node* node, int M,
                                                 ...
                                                 int& best_max, int& best_d, int& best_j)
{
    ...
    if (M >= N) {
        for (int startN = 0; startN < N; startN += REG_BAND) {
            ...
            // This is he main change, we just iterate over req_block, and now our requests are 32 elements instead of 1, which relieves LSU pressure
            for (int dblk = d; dblk <= M; dblk += REQ_BLOCK) {
                int cnt = min(REQ_BLOCK, M - dblk + 1);

                DTYPEMATRIX halo = (lane < cnt) ? prevCol[dblk + lane - startN] : (DTYPEMATRIX)0;
                DTYPEMATRIX outbuf = 0;

                for (int t = 0; t < cnt; ++t) {
                    int dd = dblk + t;

                    int hv = __shfl_sync(mask, halo, t);
                    if (lane == 0) v_a = hv;

                    process_diagonal(1, stripe_height, dd, startN - 1, M - dd + startN, -1, 0, -1);

                    int handover = __shfl_sync(mask, v_p, stripe_height);
                    if (lane == t) outbuf = handover;
                }

                if (lane < cnt) prevCol[dblk + lane - je] = outbuf;
            }
            d = M + 1;

            // --------------- Shrink phase -----------------

            for (int dblk = d; dblk <= je + M; dblk += REQ_BLOCK) {
                int cnt = min(REQ_BLOCK, je + M - dblk + 1);

                bool has_halo = (lane < cnt) && ((dblk + lane - M) <= startN);
                DTYPEMATRIX halo = has_halo ? prevCol[dblk + lane - startN] : (DTYPEMATRIX)0;
                DTYPEMATRIX outbuf = 0;
                ...
                for (int t = 0; t < cnt; ++t) {
                    ...
                    int hv = __shfl_sync(mask, halo, t);
                    if (lane == 0 && j_min <= startN) v_a = hv;

                    process_diagonal(klo, stripe_height, dd, startN - 1, M - dd + startN, -1, 0, -1);
        ...
        for (int startM = 0; startM < M; startM += REG_BAND) {
            ...
            for (int dblk = d; dblk <= is + N; dblk += REQ_BLOCK) {
                int cnt = min(REQ_BLOCK, is + N - dblk + 1);

                bool has_halo = (lane < cnt) && ((dblk + lane - startM) <= N);
                DTYPEMATRIX halo = has_halo ? topRow[dblk + lane - startM] : (DTYPEMATRIX)0;
                DTYPEMATRIX outbuf = 0;
                ...
                for (int t = 0; t < cnt; ++t) {
                    ...
                    int hv = __shfl_sync(mask, halo, t);
                    if (lane == stripe_height + 1 && dd - startM <= N) v_a = hv;

                    process_diagonal(1, stripe_height, dd, dd - ie - 2, M - ie - 1, 1, 1, 0);

                    int handover = __shfl_sync(mask, v_p, 1);
                    if (lane == t) outbuf = handover;
                }

                if (lane < cnt) topRow[dblk + lane - ie] = outbuf;
            }
            d = is + N + 1;

            // --------------- Shrink phase -----------------

            // Nothing to read here, only the handover, so this block just batches the store.
            for (int dblk = d; dblk <= ie + N; dblk += REQ_BLOCK) {
                int cnt = min(REQ_BLOCK, ie + N - dblk + 1);
                ...
                for (int t = 0; t < cnt; ++t) {
                    ...
                    process_diagonal(1, khi, dd, dd - ie - 2, M - ie - 1, 1, 1, 0);
