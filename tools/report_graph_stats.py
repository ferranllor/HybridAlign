...
def levels_of(dataset):
    ...
    return [(d, width[d], bases[d]) for d in range(num_levels)]
...
def summarise(dataset, rows, min_nodes):
    ...
    nodes = sum(w for _, w, _ in rows)
    total_bases = sum(b for _, _, b in rows)

    wide = [(d, w, b) for d, w, b in rows if w >= min_nodes]
    thin = [(d, w, b) for d, w, b in rows if w < min_nodes]

    print(f"  {dataset}: {nodes} nodes, {total_bases} bases, {len(rows)} levels, "
          f"max width {max(w for _, w, _ in rows)}")
    print(f"      width >= {min_nodes}: {len(wide):>5} levels, {sum(w for _, w, _ in wide):>6} nodes "
          f"({100.0 * sum(b for _, _, b in wide) / max(total_bases, 1):.1f}% of the work)")
    print(f"      width <  {min_nodes}: {len(thin):>5} levels, {sum(w for _, w, _ in thin):>6} nodes "
          f"({100.0 * sum(b for _, _, b in thin) / max(total_bases, 1):.1f}% of the work)")
...
def main():
    ...
    for dataset in args.datasets:
        ...
        rows += [{"dataset": dataset, "level": d, "width": w, "bases": b} for d, w, b in levels]

    with open(out, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["dataset", "level", "width", "bases"])
        writer.writeheader()
